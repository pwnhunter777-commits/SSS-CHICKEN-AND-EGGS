        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-gray-500 font-medium">
              {language === 'ta' ? 'ஹோட்டல் பாக்கி மற்றும் வரவு கண்காணிப்பு' : 'Hotel balance & payment tracking'}
            </span>
          </div>
        </div>
      ...
      {currentSelectedHotel && (
        <div className="space-y-3.5">
          ...
          >
            <div className="flex items-center justify-between pb-2.5 border-b border-white/20">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  ...
                  className="p-1.5 bg-white/20 hover:bg-white/30 active:bg-white/40 text-white rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer shadow-2xs border border-white/25"
            ...
            {/* Share Balance & Bill Receipt Action Bar */}
            <div className="pt-2.5 pb-1 border-t border-white/20 grid grid-cols-2 gap-2">
              <button
                id="btn-share-hotel-balance"
                type="button"
                onClick={handleDirectWhatsAppShare}
                disabled={isSharingBalance}
                className="py-2.5 px-3 bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 text-emerald-950 rounded-2xl text-xs font-black flex items-center justify-center gap-1.5 transition-all active:scale-98 shadow-sm cursor-pointer"
              ...
              <button
                id="btn-open-hotel-balance-slip"
                type="button"
                onClick={() => setIsBalanceSlipModalOpen(true)}
                className="py-2.5 px-3 bg-white/20 hover:bg-white/30 active:bg-white/40 text-white rounded-2xl text-xs font-black flex items-center justify-center gap-1.5 transition-all active:scale-98 border border-white/30 cursor-pointer shadow-2xs"
                title={language === 'ta' ? 'பில் போன்ற ரசீது பார்க்க & அச்சிட' : 'View Bill Slip & Print'}
              >
                <Receipt className="w-4 h-4 text-emerald-200" />
                <span className="truncate">
                  {language === 'ta' ? 'பில் ரசீது' : 'Bill Receipt'}
                </span>
              </button>
            </div>
            ...
            <div className={`grid gap-2 pt-3 border-t border-white/20 text-xs ${hotelStats.totalBalAdded > 0 ? 'grid-cols-3' : 'grid-cols-2'}`}>
              <div className="bg-black/15 rounded-2xl p-2.5">
                ...
                <span className="text-[10px] text-emerald-200 block">
                  {hotelStats.hotelBills.length} {language === 'ta' ? 'பில்கள்' : 'bills'} • {hotelStats.totalKg.toFixed(1)} {t.kg}
                </span>
              </div>
              {hotelStats.totalBalAdded > 0 && (
                <div className="bg-black/15 rounded-2xl p-2.5">
                  <div className="flex items-center gap-1 text-amber-300 text-[10px] font-bold uppercase mb-0.5">
                    ...
                    <span>{language === 'ta' ? 'பாக்கி கூட்டல்' : 'Bal Added'}</span>
                  </div>
              ...
              <div className="bg-black/15 rounded-2xl p-2.5">
                ...
                <span className="text-[10px] text-emerald-200 block">
                  {hotelStats.hotelPayments.filter((p) => p.type !== 'balance_add').length} {language === 'ta' ? 'வரவுகள்' : 'payments'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className={`text-[11px] sm:text-xs font-black px-2.5 py-1.5 rounded-xl transition-all flex items-center gap-1.5 shadow-xs active:scale-95 text-white cursor-pointer ${
                    entryMode === 'payment'
                ...
                <button
                  type="button"
                  onClick={() => setIsAddPaymentOpen(!isAddPaymentOpen)}
                  className="text-xs font-bold text-white px-2.5 py-1 rounded-xl bg-slate-700 hover:bg-slate-800 active:bg-slate-900 transition-colors shadow-xs cursor-pointer"
                >
            ...
            {isAddPaymentOpen && (
              <div className="space-y-3 pt-0.5">
                <div className="grid grid-cols-2 gap-1 p-1 bg-slate-200 rounded-2xl border border-slate-300 text-xs font-extrabold">
                  ...
                  >
                    <ArrowUpRight className="w-3.5 h-3.5" />
                    <span>{language === 'ta' ? 'பாக்கி (Bal Add)' : 'Bal Add'}</span>
                  </button>
                </div>
                ...
                <form onSubmit={handleAddPaymentSubmit} className="space-y-3 pt-0.5 animate-in fade-in duration-150">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1 flex items-center justify-between">
                      ...
                      {entryMode === 'payment' && hotelStats.balance > 0 && (
                        <button
                          ...
                          className="text-[10px] font-black text-white bg-emerald-700 hover:bg-emerald-800 px-2.5 py-1 rounded-lg transition-colors shadow-xs cursor-pointer"
                        >
                    ...
                    <input
                      ...
                      className={`w-full min-h-[48px] px-4 py-2.5 border-2 rounded-xl text-lg sm:text-xl font-black text-gray-900 outline-none transition-all leading-normal ${
                        entryMode === 'payment'
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {[500, 1000, 2000, 5000, 10000].map((preset) => (
                        <button
                          ...
                          className={`px-3 py-1.5 min-h-[36px] rounded-xl text-xs font-black transition-all active:scale-95 text-white shadow-xs cursor-pointer ${
                            entryMode === 'payment'
                    ...
                    {entryMode === 'payment' ? (
                      <div>
                        ...
                        <select
                          ...
                          className="w-full px-3 py-2 bg-emerald-50/40 border border-emerald-300 focus:border-emerald-600 rounded-xl text-xs font-bold text-gray-900 outline-none cursor-pointer"
                        >
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      type="submit"
                      className={`w-full min-h-[50px] py-3.5 px-4 text-white font-black text-sm sm:text-base rounded-2xl shadow-md flex items-center justify-center gap-2 transition-all active:scale-98 cursor-pointer ${
                        entryMode === 'payment'
                          ? 'bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900'
                          : 'bg-amber-600 hover:bg-amber-700 active:bg-amber-800'
                      }`}
                    >
                      {entryMode === 'payment' ? (
                        ...
                        <>
                          ...
                          <span>{language === 'ta' ? 'பாக்கி சேர்க்க (Save Bal Add)' : 'Save Bal Add'}</span>
                        </>
                      )}
                    </button>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="p-1.5 text-white bg-rose-600 hover:bg-rose-700 active:bg-rose-800 rounded-lg transition-colors shadow-xs cursor-pointer"
                          >
                      ...
                      >
                        ...
                        <div className="text-right flex items-center gap-1.5 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="p-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-lg text-xs font-bold transition-colors shadow-2xs cursor-pointer"
                          ...
                          <button
                            type="button"
                            onClick={() => onReprintBill(b)}
                            className="p-1.5 bg-emerald-800 hover:bg-emerald-900 active:bg-emerald-950 text-white rounded-lg text-xs font-bold transition-colors shadow-xs cursor-pointer"
                            title={t.reprint}
                          >
